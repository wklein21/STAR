...
Despite its widespread acceptance, the methodology of STAR has been criticized for issues of noncompliance due to students dropping from the program over the course of the program and inadequate record-keeping of the initial randomization process. Some objectors have also cited that the immediate measurable benefit of smaller class sizes may be exclusive to early education (Schanzenbach 2006).
...
## Data Completeness and NA Observations
...
The results of the Tukey's range test indicate that the difference between the "small" and "regular" class sizes are statistically significantly different, as well as the "small" and "regular with aide" class groups. Note that the "regular" and "regular with aide" class groups are not statistically different at the $\alpha = 0.1$ level, although this does not prevent us from determining that the "small" class size produces the highest scaled mean math scores at the 1% significance level. These results are intuitive and support much of the existing literature surrounding class size reduction, as it is highly reasonable to believe that young children might benefit from a greater share of the attention from their primary instructor. It is also possible that students in smaller class sizes are able to follow a more individualized education plan or benefit from socially engaging with a smaller, more intimate group of peers--a possibility which will be quantitatively explored in the later discussion section.
...
Another means of address a potential inequality of variances is via conducting a Levene test.

```{r, echo=F,warning=F,error=F,message=F}
res.abs=abs(model_main$residuals)
model_levene <- aov(res.abs~g1classtype + g1schid,data=STAR)
kable(cbind(tibble(` `= c("Class Type", "School ID", "Residuals")), as_tibble(summary(model_levene)[[1]]))) %>%
  kable_styling(full_width = F) %>% 
  kable_minimal(html_font = "Arial", bootstrap_options = "striped")
```

This null hypothesis for this test is that the variances are equivalent across different cells of class type and school ID. By conducting the Levene test, it is possible conclude that this null hypothesis can be rejected at the significance level of \(\alpha = 0.05\) for the school ID variable--however, with over 70 different schools, it is reasonable to observe that at least two variances would differ between schools, and failing to reject at the more conservative \(\alpha = 0.01\) is sufficient evidence that there is no major departure from the assumption of equal variances. 
...
(Note: the "g8idattn" variable, which corresponds to the students self-reporting in 8th grade whether they "only get attention when they cause trouble" merits some level of consideration on social impact, but the phrasing of "causing trouble" is sufficiently general that it may depart from this issue of socialization. Consideration for this variable is given in the appendix, as results do not differ significantly from those associated with "g8pmanoy".)
...
```{r, echo=F,warning=F,error=F,message=F}
STAR_df_attn <- STAR_raw %>%
  select(starts_with("gk") | starts_with("g1") |
           c(g8idattn,stdntid,gender,race,
             FLAGSGK,FLAGSG1,FLAGSG2,FLAGSG3,cmpstype,
             cmpsdura,yearsstar,yearssmall,)) %>%
  filter_all(any_vars(!is.na(.))) 

STAR_attn <- STAR_df_attn %>% 
  filter(!is.na(g1classtype)) %>%
  mutate(g1schid = as.factor(g1schid)) %>%
  group_by(g1tchid,g1classtype,g1schid,g1surban) %>%
  summarize(math_mean_byteacher = mean(g1tmathss,na.rm=T),
            ...
            math_10_byteacher = quantile(g1tmathss,.1,na.rm=T),
            math_90_byteacher = quantile(g1tmathss,.9,na.rm=T),
            mean_attention = mean(as.numeric(g8idattn),na.rm=T),
            mean_scamin = mean(as.numeric(g1selfconcraw), na.rm = T),
            imp_scamin = mean(as.numeric(g1selfconcraw) - as.numeric(gkselfconcraw), na.rm = T))

# aov(math_mean_byteacher ~ g1classtype + mean_attention + g1schid, data= STAR_attn)$coef

model_meanattn <- Anova(lm(math_mean_byteacher ~ g1classtype + mean_attention + g1schid, data= STAR_attn, type = "II"))
kable(cbind(tibble(` `= c("Class Type", "Mean Attention-Seeking Score", "School ID", "Residuals")), as_tibble(model_meanattn))) %>%
  kable_styling(full_width = F) %>%
  kable_minimal(html_font = "Arial", bootstrap_options = "striped")
```

As with the results for the disruptiveness score measured by the variable g8pmanoy, it appears that disruptiveness as measured by the g8idattn variable is not significant at the 1% or even 5% significance level, although it does appear to be significant at $\alpha = 0.1$. The variable ranges in value from 1 to 4, with values of 1 suggesting a given student will "strongly agree" with the statement "I only get attention when I cause trouble", and 4 suggesting that they "strongly disagree". The coefficient on this variable is 6.10, which indicates that as individuals agree more that they seek attention, they tend to receive higher test scores. While these results are marginally more significant than those previously discussed for the related measure of annoyance/disruptiveness, the ambiguity of what "seeking attention" and "causing trouble" may mean prevent this variable from being an effective measure of social disruptiveness--hence its exclusion from the main discussion section.
...
Achilles, C.M., Bain, Helen Pate et al. (2008). Tennessee's Student Teacher Achievement Ratio (STAR) project. https://doi.org/10.7910/DVN/SIWH9F, Harvard Dataverse, V1, UNF:3:Ji2Q+9HCCZAbw3csOdMNdA== [fileUNF] 
